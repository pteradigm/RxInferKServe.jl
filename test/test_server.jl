    ...
    try
        ...
        @testset "Models Endpoints" begin
            ...
            @testset "Non-existent model" begin
                response = HTTP.get("http://localhost:$port/v2/models/non_existent", status_exception=false)
        ...
        @testset "Error Handling" begin
            @testset "Invalid endpoint" begin
                response = HTTP.get("http://localhost:$port/v2/invalid", status_exception=false)
                @test response.status == 404
            end
