...
using RxInferKServe.KServeV2.kserve.v2:
    ServerLiveRequest, ServerLiveResponse, ServerReadyRequest, ServerReadyResponse
using RxInferKServe.KServeV2.kserve.v2:
    ModelReadyRequest, ModelReadyResponse, ModelMetadataRequest, ModelMetadataResponse
using RxInferKServe.KServeV2.kserve.v2:
    ModelInferRequest, ModelInferResponse, InferParameter, InferTensorContents
const InferRequestedOutputTensor =
    RxInferKServe.KServeV2.kserve.v2.var"ModelInferRequest.InferRequestedOutputTensor"
using ProtoBuf
using JSON3
...
@testset "gRPC Server and Client" begin
    ...
    try
        ...
        @testset "gRPC Inference" begin
            ...
            input_contents =
                KServeV2Types.convert_to_kserve_tensor("y", [1.0, 0.0, 1.0, 1.0, 0.0])

            # Create the input tensor with the InferInputTensor type
            input_tensor =
                RxInferKServe.KServeV2.kserve.v2.var"ModelInferRequest.InferInputTensor"(
                    "y",                    # name
                    "FP64",                 # datatype
                    [5],                    # shape
        ...
        @testset "gRPC Error Handling" begin
            ...
            @testset "Invalid inference request" begin
                ...
                @test_throws Exception KServeV2.KServeV2GRPCServer.handle_model_infer(
                    request,
                )
            end
        end
