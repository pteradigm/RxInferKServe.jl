...
using Logging
using ..KServeV2Types
using ...Models: GLOBAL_REGISTRY, get_model, create_model_instance, delete_model_instance, infer, list_models
...
function handle_v2_model_ready(request::HTTP.Request, model_name::String, model_version::Union{String,Nothing})
...
function handle_v2_model_metadata(request::HTTP.Request, model_name::String, model_version::Union{String,Nothing})
    model = get_model(model_name)
    if isnothing(model)
        return HTTP.Response(
            404,
...
function handle_v2_model_infer(request::HTTP.Request, model_name::String, model_version::Union{String,Nothing})
    try
        # Parse request body
...
function route_v2_request(request::HTTP.Request)
    ...
    return HTTP.Response(
        404,
        ["Content-Type" => "application/json"],
        JSON3.write(Dict("error" => "Endpoint not found", "path" => path, "method" => method))
    )
end
