...
function handle_model_metadata(request::ModelMetadataRequest)
    ...
    inputs_spec = get(metadata.parameters, "inputs", [Dict("name" => "data", "datatype" => "FP64", "shape" => [-1, -1])])
...
function handle_grpc_request(request::HTTP.Request)
    ...
    try
        ...
        return HTTP.Response(
            ...
            [
                "grpc-status" => status,
                "grpc-message" => sprint(showerror, e)
            ],
            ""
        )
    end
...
function start_grpc_server(; host="127.0.0.1", port=9090)
    ...
    server = HTTP.serve!(handle_grpc_request, host, port;
