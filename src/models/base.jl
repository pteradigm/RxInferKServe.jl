...
function infer(
    ...
    try
        ...
        default_params_symbols =
            Dict{Symbol,Any}(Symbol(k) => v for (k, v) in default_params)
        ...
        options_symbols = Dict{Symbol,Any}(
            Symbol(k) => v for (k, v) in options if Symbol(k) ∉ inference_options
        )
        ...
        inference_opts = merge(Dict(:iterations => iterations), options)
        ...
        if isempty(inference_data)
            ...
            throw(
                ArgumentError(
                    "Model '$(instance.model_name)' requires input data but none was provided",
                ),
            )
        end
...
function register_builtin_models()
    register_model(
        ...
        parameters = Dict{String,Any}(
            "inputs" => [Dict("name" => "y", "datatype" => "FP64", "shape" => [-1])],
            "outputs" => [Dict("name" => "theta", "datatype" => "FP64", "shape" => [2])],
        ),
    )
    ...
    register_model(
        ...
        parameters = Dict{String,Any}(
            "inputs" => [Dict("name" => "y", "datatype" => "FP64", "shape" => [-1])],
            "outputs" => [Dict("name" => "mu", "datatype" => "FP64", "shape" => [1])],
        ),
    )
    ...
    register_model(
        ...
        parameters = Dict{String,Any}(
            ...
            "obs_noise" => 1.0,
            "inputs" => [Dict("name" => "y", "datatype" => "FP64", "shape" => [-1])],
            "outputs" => [Dict("name" => "states", "datatype" => "FP64", "shape" => [-1])],
        ),
    )
end
